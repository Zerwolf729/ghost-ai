...
versions=$(curl -fsSL --retry 3 --retry-delay 1 "$API_URL" | node -e "
  let d='';
...
if [[ -z "$latest" ]]; then
  echo "ERROR: No dated spec versions found at $API_URL" >&2
  exit 1
fi
...
echo ""
echo "TAGS:"
curl -fsSL --retry 3 --retry-delay 1 "${RAW_BASE}/${latest}" | node "$(dirname "$0")/extract-tags.js"
