...
/**
 * Hard wall-clock cap per run attempt — prevents hanging runs.
 * Sized well above observed generation times (~90s on the free-tier model)
 * so a slow-but-successful generation is never aborted mid-flight.
 */
...
function friendlyError(err: unknown): string {
  if (err instanceof Error) {
    ...
    // Trim to first sentence; drop stack traces.
...
export const designAgent = task({
	...
	run: async (payload: { prompt: string; roomId: string; userId: string }) => {
		...
		// Request timeout — abort if AI call exceeds the time budget.
		...
		try {
			...
			// Strip markdown fences the free-tier model sometimes emits.
			const cleaned = text.replace(/^```(?:json)?\s*/i, "").replace(/\s*```$/i, "").trim();
			let object: z.infer<typeof canvasSchema>;
			try {
				object = JSON.parse(cleaned) as z.infer<typeof canvasSchema>;
			} catch (parseErr) {
				console.error("Design agent JSON parse error:", parseErr, "raw text:", cleaned.slice(0, 500));
			...
			const flowNodes = object.nodes.map((n) => {
				...
				// Validate position — NaN/Infinity guard so React Flow never receives
				// garbage coordinates that break geometry (and thus label/routing).
				...
				// Min width from label length: max(chars*8 + 32, shape floor, AI value).
			...
			// Center-point lookup for deterministic handle assignment.
			...
			// Deduplicate edges between the same source/target pair — keep ONE
			// label per connector (the first emitted), drop overlapping parallel
			// edges so no edge carries more than one label.
			...
			(object.edges ?? [])
				...
				.forEach((e, i) => {
					...
					// ONE label per connector: keep the first emitted label only.
					if (!edgeMap.has(key)) {
			...
			// Cancellation is expected during normal shutdown — skip user-facing error.
			if (controller.signal.aborted || (error as { name?: string })?.name === "AbortError") {
				// Throw so Trigger.dev marks the attempt as failed and retries —
				// a normal return would be recorded as a Success, which is exactly
				// the "AI run failed" UI + "Success" in Trigger.dev mismatch we saw.
				console.warn("Design agent attempt aborted (timeout). Retrying...");
				throw new Error("AI request timed out. Retrying...");
			}

			const msg = friendlyError(error);
			// Surface the failure to the room so the UI's loading state always
			// resolves to a terminal state, even when this attempt is mid-retry.
			await liveblocks.broadcastEvent(roomId, { type: "AI_STATUS", status: "failed", message: msg });
			// Re-throw so Trigger.dev retries with exponential backoff.
			// The attempt number is tracked by Trigger.dev; each retry re-broadcasts "started".
