...
const SHAPE_ICONS: Record<Shape, LucideIcon> = {
...
function PreviewTriangle() {
  return (
    <svg width="100%" height="100%" viewBox="0 0 100 100" preserveAspectRatio="none">
      <polygon points="50,5 95,95 5,95" fill={PREVIEW_FILL} stroke={PREVIEW_STROKE} strokeWidth="2" strokeLinejoin="round" />
    </svg>
  );
}
...
function PreviewDatabase() {
  return (
    <svg width="100%" height="100%" viewBox="0 0 100 100" preserveAspectRatio="none">
      <ellipse cx="50" cy="20" rx="40" ry="15" fill={PREVIEW_FILL} stroke={PREVIEW_STROKE} strokeWidth="2" />
      <rect x="10" y="20" width="80" height="60" fill={PREVIEW_FILL} />
      <line x1="10" y1="20" x2="10" y2="80" stroke={PREVIEW_STROKE} strokeWidth="2" />
      <line x1="90" y1="20" x2="90" y2="80" stroke={PREVIEW_STROKE} strokeWidth="2" />
      <ellipse cx="50" cy="80" rx="40" ry="15" fill={PREVIEW_FILL} stroke={PREVIEW_STROKE} strokeWidth="2" />
    </svg>
  );
}

function previewBorderRadius(shape: Shape): string {
    ...
    <div style={{ width, height, pointerEvents: "none" }}>
      {isSvg ? (
        <>
          ...
          {shape === "triangle" && <PreviewTriangle />}
          {shape === "database" && <PreviewDatabase />}
        </>
      ) : (
        <div
          style={{
...
export function ShapePanel({
  ...
  function handleDragStart(event: React.DragEvent, shape: Shape) {
  ...
  function handleDrag(event: React.DragEvent, shape: Shape) {
